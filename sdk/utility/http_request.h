...
public:
    ...
    /// Get request url.
    CDuiString GetURL() const {return m_header.m_url;}
...
private:
    ...
    CDuiString   m_tmp_host ; // parsed for thread
    CDuiString   m_tmp_object ;
...
private:
    static DWORD WINAPI http_connect_proc (LPVOID lpParameter)
    {
		...
		HINTERNET ires = InternetConnect (p->m_session, p->m_tmp_host, nPort, NULL, NULL, INTERNET_SERVICE_HTTP, 0, 0);
        p->m_connect.Attach (ires) ;
    ...
    static void http_send_request_proc (FCHttpRequest* p)
    {
        ...
        // range header
        CDuiString   th = p->m_header.m_header ;
        if (p->m_header.m_start)
        {
			th.Format(_T("%sRange: bytes=%d-\r\n"), th.GetData(), p->m_header.m_start);
        }
    ...
    static DWORD WINAPI http_request_proc (LPVOID lpParameter)
    {
        ...
        while (true)
        {
            ...
            {
				char s[MAX_PATH] = {0};
				sprintf_s(s, "sdp_test2 update http_request_proc bReadResult=%d,dw=%ld", bReadResult, dw);
...
private:
    ...
    HINTERNET CreateSessionHandle() const
    {
        ...
        if (m_header.m_proxy_ip.GetLength())
        {
			...
			char s[MAX_PATH] = {0};
			sprintf_s(s, "%s:%d", m_header.m_proxy_ip.GetData(), m_header.m_proxy_port) ;
