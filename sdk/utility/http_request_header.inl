...
public:
    ...
    */
    ...
    CDuiString   m_proxy_ip ; ///< proxy server IP
    int       m_proxy_port ; ///< proxy server port, default is 80
    CDuiString   m_proxy_username ; ///< user name to login the proxy if need
    CDuiString   m_proxy_password ; ///< password to login the proxy if need
    ...
    void GetUrlComponent (CDuiString& sHost, CDuiString& sObject, INTERNET_SCHEME& nScheme) const
    {
        URL_COMPONENTS   uc ;
    ...
    void AddPostData (const CDuiString& s)
    {
    ...
    void AddMultipartFormData (const CDuiString& sName, const CDuiString& sValue)
    {
        AddMultipartFormData (sName, (LPCSTR)sValue, sValue.GetLength(), NULL) ;
    }
    /// Add buffer param.
    void AddMultipartFormData (const CDuiString& sName, const void* pValue, size_t nValueSize, LPCSTR sFileName)
    {
        CDuiString s1;
		s1.Format(_T("--%s\r\nContent-Disposition: form-data; name=\"%s\""), DEFAULT_BOUNDARY().GetData(), sName.GetData());
        if (sFileName)
        {
			CDuiString s2;
    ...
    void EndMultipartFormData()
    {
        ...
        m_header += _T("Accept: */*\r\n") ;
		m_header.Format(_T("%sContent-Type: multipart/form-data; boundary=%s\r\nContent-Length: %d\r\n"), m_header.GetData(), DEFAULT_BOUNDARY().GetData(), (int)m_post_data.size());
    }
    //@}

private:
    // Get user-agent used by IE.
    static CDuiString GetSystemUserAgent()
    {
        DWORD  n = 1024 ;
        char   t[1024] = {0} ;
    ...
    static CDuiString DEFAULT_BOUNDARY()
    {
